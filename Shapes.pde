...
//    HEXAGON  (n=6): regular hexagons are NOT rep-tiles, so single-scale
//                    only (hex tiles are never subdivided).
//
...
import processing.awt.PGraphicsJava2D;     // for polygon clipping via Graphics2D
...
// Hexagon: single-scale, so every tile is FULLY CONNECTED (each of the 6
...
boolean canSubdivide(Tile t) {
  return t.n != 6 && t.depth < maxDepth;     // hexagons never subdivide
}
...
void drawPolyTile(Tile t, color fg, color bg) {
  ...
  // Hexagon skips the clip so its PROJECT-capped bands (see drawPolyConn) can
  ...
  // Hexagons are single-scale, so they need no wings; their bands instead use
  // PROJECT caps (see drawPolyConn) to overlap across shared edges.
  if (winged && n != 6) {
    ...
    fill(fg);                                          // edge-midpoint discs, r = side/6
...
void drawPolyConn(int i, int j, int n,
  ...
  // hexagon bands overlap across edges (no clip there); square/triangle end
  // flush at the edge and rely on wings to bridge the seam.
  strokeCap(n == 6 ? PROJECT : SQUARE);
  noFill();
